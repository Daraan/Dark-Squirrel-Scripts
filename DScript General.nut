...
Basically it's to prevent midway triggering of levers 
which allows to skip the opposite message and will trigger the last one again.
*/
#########################################
{
...
#########################################
/*
Has all the StdButton features - even TrapControlFlags work as well as the DRelayTrap features, 
so basically this can save some script markers which only wait for a Button TurnOn.

Additionally:
	Once will lock the Object.
	And if the button is LOCKED the joint will not activate 
	and the Schema specified by DStdButtonLockSound will be played, 
	by default "noluck" the wrong lockpick sound.

NOTE: As this is a DRelayTrap script as well it can be activated via TurnOn; 
      but the Default message is set to null.
	  sending this message will bypass the Lock check and TrapControlFlags.
######################################### */
	{
###StdController
...
// /--
####################################################################
class DHitScanTrap extends DTrigger
####################################################################
/*When activated will scan if there is one object / solid between two objects.
Imagine it as a scanning laser beam between two objects DHitScanTrapFrom and DHitScanTrapTo, 
the script object is used as default if none is specified. 

If the From object is the player the camera position is used 
if the To object is also the player the beam will be centered at the players view 
- for example to check if hes exactly facing something (but it should be a rather large object).

The Object that was hit will receive the message specified by DHitScanTrapHitMsg.
...
// No need to always recreate these objects each turn
hobj = object()
hloc = vector()
...
#		|--  Actual Scan --|		
		local result = (1 + Engine.ObjRaycast(vfrom, vto, hloc, hobj, 
				FALSE, 																					// ShortCircuit
				DGetParamRaw(_script + "RenderedOnly",2,DN) + DGetParamRaw(_script + "IgnoreAI",0,DN),	// Filter Flags
				from, to)																				// Ignore Objects
...
#		|-- Restoring Ignore set --|
	...
	function DoOff()
	{
		...
		if (IsDataSet(_script+"TInfRepeat")){
			if (IsDataSet(_script+"TDelayTimer"))
				KillTimer(ClearData(_script+"TDelayTimer"))
			ClearData(_script+"TInfRepeat")
			_script = _script +"T"							// DeRegister uses _script, need to rename to check if T is registered.
			::DHandler.PerFrame_DeRegister(this)			// Even if not set will do no harm.
			_script = _script.slice(0,-1)					// Aaand reset. 
		}
...
/*
Targeted AIs will semi ignore you.
Depending on your action and mode set.
See Forum post for a more detailed explanation.
 */
#########################################
{
