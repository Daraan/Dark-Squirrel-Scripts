...
/* Same as InventoryMaster but only displays its own contents. */
{
	...
	// Idea to remove the Inventory if it is empty, but is if the last item is temporarily given to the player
	function XOnContainer(){
...
/* Replacement for the standard LootSounds scripts. Appends / TotalLoot, behind Loot objects.*/
	...
	TotalLoot = int_ref()						// Non scalar members are shared between instances
	...
	function GetLootFromObj(obj){
		...
		foreach (field in fields){
			TotalLoot.constructor(TotalLoot.tointeger() + ::Property.Get(obj,"Loot",field))
		}
	}

	function GetAllLoot(){
		if (TotalLoot.tointeger())
			return TotalLoot.tointeger()
	...
	function OnMessage(){
		...
		if (::startswith(mssg, "Leaving") || ::startswith(mssg,"Reached") || mssg == "Canceled"){
			...
			if (targets.len()){
				...
				DN["On" + mssg + "Target"] <- targets 		// In case off ScriptParams fallback have to use these.
				_script 	= "On"+ mssg
				SourceObj	 = message().data3				// Needs to be set manually.
				TriggerMessages(!leave, DN, message().data, message().data2, SourceObj)		// leave = 1 => false, 0=>true
