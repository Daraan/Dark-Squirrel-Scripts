...
| T-120 | P1 | `DScript Core.nut:924,926,929,939` | `DScript.DeleteQVar` calls `Quest.BinGetTable` / `BinGet` / `BinExists` / `Get` **without `::`** — inside a DScript-table function the bare name resolves to `this.Quest`, which since `DScript.Quest <-` (Core:2956, the trigger registry) shadows the engine service. Line 929 runs unconditionally, so **every `DeleteQVar` call throws** "the index 'BinExists' does not exist" — `DTrapDeleteQVar` is dead. The sibling `GetQVar`/`SetQVar` already use `::Quest` throughout. Found by `tools/dstest/tests/test_dscript_qvar.nut` | prefix the four calls with `::` | ☐ |
| T-29 | P1 | `DScript Core.nut:1381-1382` | `+` combine operator: `::split(str,"+")` then `ar.remove(0)` — written for a split that keeps the leading empty token, but NewDark `split()` **drops** empty tokens (the same premise T-20's fix reindexed for), so `remove(0)` deletes the **first real entry**: `"+A+B"` applies only `B`. Hits every `+` user, e.g. `DCopyPropertyTrapProperty="+P1+P2"` silently skips `P1`. Found by `tools/dstest` after aligning its `split()` shim with the engine's drop-empties behavior. NOTE: T-20's fix and this row share the drop-empties premise — one DromEd check of `split("]a]b","]")` settles both; if the engine in fact keeps empties, revert T-20's reindex instead and drop this row | delete the `ar.remove(0)` line | ☐ |
| T-94 | P2 | `DScript Core.nut:1414` (`{` distance op), `DScript_ModdingTools.nut:266, 330` | NewDark `split()` matches multi-char separators as one literal substring, not a char set — these calls never split, the whole string came back as one token. Found in-game 2026-08-13 via the `<x,y,z>` vector operator (fixed same day) | `{` header now scanned by hand; ModdingTools sites use a new keep-empties `DSplitSet` helper (their `i=1` loop starts expect the leading empty token). The fourth candidate at `:722` is inside the dead `/* DImportObj */` comment block — no fix needed. Raw bracket census of `check_files.py` shifted on Core (comment/string bytes); comment-and-string-aware balance verified 0/0/0. **Reopened and re-closed 2026-08-18** — the sweep missed four more sites: `Core:2712` (`"=;"`, DHub sub-DesignNotes), `Core:2894` (`":;"`, `InitQVarFromProp`) and `SFX:1169, 1201` (`"[,]"`, `DTweqDevice` joints, see T-113). All four now use the new `::DScript.SplitAny` helper | ☑ pending DromEd verification |
...
A second, independent review pass over the same branch produced T-111…T-119. Its most useful
finding was against the T-95 fix in this very table: removing the empty tokens *inside* the scan
loop shifted the array under the absolute indices that loop had already recorded for deferred
removal, so the removal then ran one past the end. The empties are now dropped in a pass before the
scan. A fix landing is not the same as a fix being right — re-read the repaired code, not the
ticket describing it.

**ID ranges:** T-105…T-110 belong to the `dfile`/`dblob` search work in Group K, which was landing
in parallel. Check the whole file before claiming a new number.
